...
#define TEXT_COLS (COLS - LINE_NUM_WIDTH)
#define ROWS 23
...
static char tbuf[MAX_SIZE];
static int  tlen = 0;
static int  cx = 0, cy = 0;
static int  scroll = 0;
static char tfilename[12];
...
static void tuze_draw(void) {
    ...
    for (int r = 0; r < ROWS; r++) {
        ...
        if (line < lines) {
            ...
            for (int p = 0; p < pad; p++)
                vga_set(r, p, ' ', 0x08);
            for (int p = 0; p < ti; p++)
                vga_set(r, pad + p, tmp[ti - 1 - p], 0x08);
            vga_set(r, 3, ' ', 0x08);
        }
        ...
        while (pos < tlen && tbuf[pos] != '\n' && col < TEXT_COLS) {
            vga_set(r, LINE_NUM_WIDTH + col, tbuf[pos], WHITE);
            col++;
            pos++;
        }
    }
    ...
    if (cur_row >= 0 && cur_row < ROWS)
        vga_set(cur_row, LINE_NUM_WIDTH + cx,
                cx < line_len(cy) ? tbuf[line_start(cy) + cx] : ' ', 0x70);
}
